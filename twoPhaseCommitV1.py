...
class twoPhaseCommit:
    ...
    def txnSuccessCallback(self):
        print("Transaction was successfully completed by the server")
    ...
    # For now we just flip a coin to determine if a txn has failed or not, as that is out of the scope for our project. Being realistic, 
    ...
    def handle_commit_req(self, trans_id, server, cp_msg): print ("SERVER: Commit request has been received")	

    #'Txn wasnt completed by server'
    def txnFailCallback(self, trans_id):
        print("Transaction %d was NOT completed"%trans_id)
